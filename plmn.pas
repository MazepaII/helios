  ...
  function pul;
    ...
    function v1(p,t:extended):extended;
      ...
      begin
        ...
        s:=0;
        for i:=1 to 34 do s:=s+na[i]*ia[i]*fn(pim,ia[i]-1)*fn(tam,ja[i]);
    ...
    function h1(p,t:extended):extended;
      ...
      begin
        ...
        s:=0;
        for i:=1 to 34 do s:=s+na[i]*ja[i]*fn(pim,ia[i])*fn(tam,ja[i]-1);
        h1:=s*0.461526*1386;
    end;
    function s1(p,t:extended):extended;
      ...
      begin
        ...
        for i:=1 to 34 do s:=s+na[i]*ja[i]*fn(pim,ia[i])*fn(tam,ja[i]-1);
        s:=s*1386/(t+273.15);
        for i:=1 to 34 do s:=s-na[i]*fn(pim,ia[i])*fn(tam,ja[i]);
        s1:=s*0.461526;
    end;
    function t1(p,h:extended):extended;
      ...
      begin
        ...
        s:=0;
        for i:=1 to 20 do s:=s+na2[i]*fn(pim,ia2[i])*fn(etm,ja2[i]);
        t1:=s-273.15;
    end;
    function v2(p,t:extended):extended;
      ...
      begin
        ...
        s:=1/pim;
        for i:=1 to 43 do s:=s+nb[i]*ib[i]*fn(pim,ib[i]-1)*fn(tam,jb[i]);
    ...
    function h2(p,t:extended):extended;
      ...
      begin
        ...
        s:=0;
        for i:=1 to 9 do s:=s+n0[i]*j0[i]*fn(ta,j0[i]-1);
        for i:=1 to 43 do s:=s+nb[i]*jb[i]*fn(pim,ib[i])*fn(tam,jb[i]-1);
        h2:=s*0.461526*540;
    end;
    function s2(p,t:extended):extended;
      ...
      begin
        ...
        for i:=1 to 43 do s:=s+nb[i]*jb[i]*fn(pim,ib[i])*fn(tam,jb[i]-1);
        s:=s*ta-ln(pim);
        for i:=1 to 9 do s:=s-n0[i]*fn(ta,j0[i]);
        for i:=1 to 43 do s:=s-nb[i]*fn(pim,ib[i])*fn(tam,jb[i]);
        s2:=s*0.461526;
    end;
    function p3(r,t:extended):extended;
      ...
      begin
        ...
        s:=nc[1]/de;
        for i:=2 to 40 do s:=s+nc[i]*ic[i]*fn(de,ic[i]-1)*fn(ta,jc[i]);
    ...
    function h3(r,t:extended):extended;
      ...
      begin
        ...
        s1:=0;
        for i:=2 to 40 do s1:=s1+nc[i]*jc[i]*fn(de,ic[i])*fn(ta,jc[i]-1);
        s2:=nc[1]/de;
        for i:=2 to 40 do s2:=s2+nc[i]*ic[i]*fn(de,ic[i]-1)*fn(ta,jc[i]);
    ...
    function s3(r,t:extended):extended;
      ...
      begin
        ...
        s:=0;
        for i:=2 to 40 do s:=s+nc[i]*jc[i]*fn(de,ic[i])*fn(ta,jc[i]-1);
        s:=s*ta-nc[1]*ln(de);
        for i:=2 to 40 do s:=s-nc[i]*fn(de,ic[i])*fn(ta,jc[i]);
        s3:=s*0.461526;
    end;
    function t2a(p,h:extended):extended;
      ...
      begin
        ...
        s:=0;
        for i:=1 to 34 do s:=s+nb2[i]*fn(pim,ib2[i])*fn(etm,jb2[i]);
        t2a:=s-273.15;
    end;
    function t2b(p,h:extended):extended;
      ...
      begin
        ...
        s:=0;
        for i:=1 to 38 do s:=s+nb3[i]*fn(pim,ib3[i])*fn(etm,jb3[i]);
        t2b:=s-273.15;
    end;
    function t2c(p,h:extended):extended;
      ...
      begin
        ...
        s:=0;
        for i:=1 to 23 do s:=s+nb4[i]*fn(pim,ib4[i])*fn(etm,jb4[i]);
        t2c:=s-273.15;
    end;
    function t2sa(p,s:extended):extended;
      ...
      function fn2(x,n:extended):extended;
        ...
        begin
          f:=frac(n);
          m:=0;
          if (f>0.4) and (f<0.6) then m:=1;
          if (f>0.2) and (f<0.3) or (f>0.7) and (f<0.8) then m:=2;

          if m=0 then fn2:=fn(x,round(n))
          else if m=1 then fn2:=sqrt(fn(x,round(2*n)))
          else fn2:=sqrt(sqrt(fn(x,round(4*n))));
      end;
      begin
        ...
        s2:=0;
        for i:=1 to 46 do s2:=s2+nb5[i]*fn2(pim,ib5[i])*fn(sig,jb5[i]);
        t2sa:=s2;
    end;
    function t2sb(p,s:extended):extended;
      ...
      begin
        ...
        s2:=0;
        for i:=1 to 44 do s2:=s2+nb6[i]*fn(pim,ib6[i])*fn(sig,jb6[i]);
        t2sb:=s2;
    end;
    function t2sc(p,s:extended):extended;
      ...
      begin
        ...
        s2:=0;
        for i:=1 to 30 do s2:=s2+nb6[i]*fn(pim,ib6[i])*fn(sig,jb6[i]);
        t2sc:=s2;
    end;
    function r3(p,t:extended):extended;
      var y,y1,y2,z:extended;
      begin

        y:=1/v2(pg(t),t);
        z:=p3(y,t);
        if z>p then begin y2:=y;
          while z>p do begin
            y:=y/1.01;
            z:=p3(y,t);
          end;
    ...
    function t3(p,h:extended):extended;
      var y,y1,y2,z,f1,f2,t2:extended;
      begin
    ...
    function r3f(t:extended):extended;
      ...
      begin
        ...
        y:=47*sqrt(x);
        if x<1 then y:=y+8*x
        else if x<4 then y:=y+5+2.5*x
        else if x<9 then y:=y+15
        else if x<4 then y:=y+20-0.75*x;
        r3f:=y+322;
    end;
    ...
    function mu(p,t:extended):extended;
      var r,u,v,A,B,C,D,E,mu0,k,kt,u1,u2,v4,r0,rm,s:extended;
      begin
        s:=647.27/(x+273.15);
        u:=s-1.0;
        u1:=1/s-1;
    ...
    function la(p,t:extended):extended;
      var r,u,v,w,A,B,C,D,E,F:extended;
      begin
        r:=647.27/(x+273.15);
        u:=r-1.0;
        v:=3.1416999e-3/p;
    ...
    begin
      ...
      9:  pul:=235.6*exp(1.256*ln((374.15-x)/647.3))*(1-0.625*((374.15-x)/647.3));
